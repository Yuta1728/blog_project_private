...
def create_app():
    ...
    【処理の流れ】
      ...
      STEP 2. ProxyFix を適用（リバースプロキシ対応）
      STEP 3. SECRET_KEY を設定（本番未設定なら起動拒否）
      STEP 4. セッション Cookie の属性を設定
      STEP 5. アップロードサイズを制限（30MB）
      STEP 6. データベース接続 URL を設定
      STEP 7. 拡張機能（db / migrate / login_manager）を初期化
      STEP 8. CSRF 保護を適用
      STEP 9. Flask-Login を詳細設定（404 偽装 / user_loader）
      STEP 10. セキュリティヘッダーを全レスポンスに付与
      STEP 11. カスタムエラーハンドラー（413）を登録
      STEP 12. Blueprint を登録して app を返す
    """
...
#
# 【処理の流れ】
#   STEP 1. create_app() でアプリを生成
#   STEP 2. 本番環境かどうかを判定
#   STEP 3. 「本番ではない」かつ「FLASK_DEBUG=1」のときだけデバッグ有効
#   STEP 4. app.run() でサーバー起動
