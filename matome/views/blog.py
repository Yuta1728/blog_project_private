...
def genre_list():
    ...
    【修正の背景】
    以前はジャンルのグループ分けと各ジャンルを genre.html にベタ書きし、
    このビューは sorted(DEFAULT_GENRES) を渡すだけだった。
    そのため constants.py にジャンルを足しても genre.html を手で直さない限り
    一覧に反映されず、ユーザーが独自作成したジャンルも永遠に表示されなかった。

    【この修正でやること】
    グループ構造を constants.GENRE_GROUPS に一元化し、そのまま genre.html に渡す。
    さらに「DB に実在するがプリセットに無いジャンル（＝ユーザー独自作成分）」を
